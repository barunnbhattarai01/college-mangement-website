...
function About() {
  ...
  return (
    ...
    <div className="border border-gray-300 shadow-lg rounded-xl w-full max-w-md p-8 bg-white dark:bg-gray-800 ml-auto mr-auto">
  ...
  <div className="space-y-4 text-2xl text-gray-700 dark:text-gray-200">
    {data.map((item, index) => (
      <>
   <p key={index}> <span className="text-amber-500 font-semibold" key={index}>{item.icon}</span>  
        {item.label}: <span className="text-amber-500 font-semibold" key={index}>{item.value}</span>
      </p>
      </>
    ))}
  </div>
